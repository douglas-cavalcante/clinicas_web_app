...
import { getProcredureReportRequest } from '~/store/modules/report/procedures/actions';
...
export default function ProceduresReport() {
  ...
  function getPdf(pageOrientation) {
    const labels = ['Profissional', 'Procedimento', 'Quantidade', 'Total'];

    const data = [];

    proceduresReport.data.map(item => {
      data.push([item.profissional, item.procedimento, item.qtd, item.total]);
    });
  ...
  return (
    <>
      <Header title="Relatório de procedimentos por professional" />
